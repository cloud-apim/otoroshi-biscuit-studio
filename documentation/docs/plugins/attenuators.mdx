...
The newly attenuated token can then be injected into the desired location, whether headers, cookies, or query parameters. You can also use the expression language to get the token using the expression `${ctx.attenuated_biscuit}`
