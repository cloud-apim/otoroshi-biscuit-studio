...
this plugin can be used to implement the OAuth2 `client_credentials` flow with the `access_token` being a biscuit.
this plugin is a `Backend` for any route you want. It expects a `POST` with a typical OAuth2 `client_credentials` flow payload.
