...
| `user_name` (if present) | The authenticated username of the requester. | `username("alice")` |
| `user_email` (if present) | The authenticated email of the requester. | `email("alice@example.com")` |
| `req_headers` | The HTTP headers sent with the request, including various key details like authorization tokens, user-agent, etc. | `req_headers("custom-header", "<value>")`<br />`req_headers("remote-address", "192.168.1.1:12345")`<br />`req_headers("authorization", "Bearer <token>")`<br />`req_headers("request-start-time", "<timestamp>")`<br />`req_headers("accept-encoding", "gzip, deflate")` |
