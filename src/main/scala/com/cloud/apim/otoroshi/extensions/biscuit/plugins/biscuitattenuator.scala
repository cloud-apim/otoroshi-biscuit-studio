...
class BiscuitTokenAttenuator extends NgRequestTransformer {
  ...
  override def name: String = "Cloud APIM - Biscuit Token attenuator"
                      ...
                      case Right(biscuitUnverified) =>
                        ...
                        Try(biscuitUnverified.verify(publicKey)).toEither match {
                          ...
                          case Right(biscuitToken) => {
                            ...
                            ctx.otoroshiRequest
                              .copy(
                                headers = ctx.otoroshiRequest.headers ++ Map("X-Otoroshi-Biscuit" -> attenuatedToken.serialize_b64url())
                              )
                              .right
                          }
                        }
